...
import org.hobbit.core.tool.support.IMultiOutputStream;
import org.hobbit.core.tool.support.ImagePosition;
...
public class ImageUtil {
  ...
  public static void zoomFixed(BufferedImage src, OutputStream output, String type, int height,
      int width, boolean bb, Color fillColor) {
    try {
      double ratio;
      ...
      if (bb) {
        ...
        if (width == itemp.getWidth(null)) {
          g.drawImage(itemp, 0, (height - itemp.getHeight(null)) / 2, itemp.getWidth(null),
              itemp.getHeight(null), fill, null);
        } else {
  ...
  public static void sliceWithSize(BufferedImage src, IMultiOutputStream mos, String type,
    ...
    try {
      ...
      if (srcWidth > destWidth && srcHeight > destHeight) {
        ...
        // 切片横向数量
        int cols =
            (srcWidth % destWidth == 0) ? (srcWidth / destWidth) : (srcWidth / destWidth + 1);
        // 切片纵向数量
        int rows =
            (srcHeight % destHeight == 0) ? (srcHeight / destHeight) : (srcHeight / destHeight + 1);
        // 循环建立切片
        // 改进的想法:是否可用多线程加快切割速度
        for (int i = 0; i < rows; i++) {
          for (int j = 0; j < cols; j++) {
  ...
  public static void textStamp(BufferedImage src, OutputStream output, String type, String text,
    ...
    try {
      ...
      // 在指定坐标绘制水印文字
      ImagePosition boxPos = new ImagePosition(width, height, calcTextWidth(text) * font.getSize(),
          font.getSize(), position);
