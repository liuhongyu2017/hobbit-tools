 ...
 */
public abstract class HobbitBeanCopier {
  ...
  private static final Signature IS_ASSIGNABLE_VALUE =
      TypeUtils.parseSignature("boolean isAssignableValue(Class, Object)");
  /**
   * The map to store {@link HobbitBeanCopier} of source type and class type for copy.
   */
  private static final ConcurrentMap<HobbitBeanCopierKey, HobbitBeanCopier> BEAN_COPIER_MAP =
  ...
  public static class Generator extends AbstractClassGenerator<Object> {
    ...
    @Override
    public HobbitBeanCopier create(Object key) {
      return (HobbitBeanCopier) super.create(key);
    }
    ...
    public void generateClass(ClassVisitor v) {
      ...
      for (PropertyDescriptor setter : setters) {
        ...
        CopyProperty targetIgnoreCopy =
            ReflectUtil.getAnnotation(target, propName, CopyProperty.class);
        // set 上有忽略的 注解
    ...
    private static void invokeWrite(CodeEmitter e, MethodInfo write, Method writeMethod,
        boolean nonNull, Label l0) {
      // 返回值，判断 链式 bean
     ...
     */
    public void generateClassFormMap(ClassEmitter ce, CodeEmitter e, Type sourceType,
        Type targetType) {
      // 2018.12.27 by L.cm 支持链式 bean
      PropertyDescriptor[] setters = ReflectUtil.getBeanSetters(target);
