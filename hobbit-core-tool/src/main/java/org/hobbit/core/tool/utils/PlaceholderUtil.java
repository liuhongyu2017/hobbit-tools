...
public class PlaceholderUtil {
  ...
  /**
   * 解析带有指定占位符的模板字符串，默认占位符为前缀：${ 后缀：}<br/> <br/> 如：template = category:${}:product:${}<br/> values =
   * {"1", "2"}<br/> 返回 category:1:product:2<br/>
   *
  ...
  /**
   * 替换模板中占位符内容，占位符的内容即为map key对应的值，key为占位符中的内容。<br/> <br/> 如：content =
   * product:${id}:detail:${did}<br/> valueMap = id -> 1; pid -> 2<br/> 经过解析返回
